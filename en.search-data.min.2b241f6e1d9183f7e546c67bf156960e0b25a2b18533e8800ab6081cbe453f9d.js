...
实战 5: 六边形架构
`}),e.add({id:11,href:"/deancn/wiki/arch/ldc/",title:"Ldc",section:"系统架构",content:`浅谈双十一背后的支付宝LDC架构和其CAP分析
https://tbwork.org/2019/11/10/ant-ldc-arch/
`}),e.add({id:12,href:"/deancn/wiki/arch/question/",title:"Question",section:"系统架构",content:`在新项目开发中如何？
在老项目维护中如何？
在技术构架演化中如何？
`}),e.add({id:13,href:"/deancn/wiki/dev/coding-style/document/",title:"Document",section:"Coding Style",content:` 中文文案排版指北 # 统一中文文案、排版的相关用法，降低团队成员之间的沟通成本，增强网站气质。
...
`}),e.add({id:15,href:"/deancn/wiki/dev/emoji-list/",title:"Emoji List",section:"Development Handbook",content:` emoji-list # emoji表情列表
...
Workflow:
action.yml 中配置, 启用自定义的 action , 增加 input 变量作为 docker run 参数 调用自定义的 action , 从 env 中获取 GITHUB_EVENT_PATH, 读取 event.json 全部内容 代码中进行自定义操作 设置输出, 如 echo \u0026ldquo;::set-output name=time::\$time\u0026rdquo; `}),e.add({id:17,href:"/deancn/wiki/dev/github/fork/",title:"Fork",section:"Git and Github",content:` Github 技巧 # Gitlab 或 Github Fork 后如何同步源的新更新内容？ # 给 fork 配置远程库 # 查看远程状态 git remote -v
...
`}),e.add({id:18,href:"/deancn/wiki/dev/github/git_branch_manage/",title:"Git Branch Manage",section:"Git and Github",content:` Git 分支的约束和管理 # 来源： dxyoo7
...
`}),e.add({id:19,href:"/deancn/wiki/dev/github/how_to_github_pull_request/",title:"How to Github Pull Request",section:"Git and Github",content:` 如何参与贡献: # Github 提交 PR, 地址: github.com/airdb/fun Github 提交 issues, 地址: github.com/airdb/fun/issues 写邮件给 info@airdb.com Github 如何提交 PR # step 1:
...
`}),e.add({id:20,href:"/deancn/wiki/dev/github/issue/",title:"Issue",section:"Git and Github",content:` Github Issue 的用法 # Reference
...
`}),e.add({id:23,href:"/deancn/wiki/dev/go/pprof/memory_leak/",title:"Memory Leak",section:"Golang",content:` go pprof与线上事故：一次成功的定位与失败的复现
...
`}),e.add({id:24,href:"/deancn/wiki/dev/go/serverless-tencent/",title:"Serverless Tencent",section:"Golang",content:` 腾讯云 Serverless # 讨论区： https://github.com/serverless/serverless-tencent/discussions
...
部署 # SERVERLESS_PLATFORM_VENDOR=tencent GLOBAL_ACCELERATOR_NA=true sls deploy --stage test 查看日志 # SERVERLESS_PLATFORM_VENDOR=tencent GLOBAL_ACCELERATOR_NA=true sls logs --tail --stage test `}),e.add({id:25,href:"/deancn/wiki/dev/go/tools/",title:"Tools",section:"Golang",content:` Golang Command Line tools # Linux pipes in Golang
使用Go语言在MacOS创建一个自定义的命令行工具
你应该定期更新 Homebrew
`}),e.add({id:26,href:"/deancn/wiki/dev/standard/case/",title:"Case",section:"Development Handbook",content:` 好好写代码 # Bad Cases # Please don\u0026rsquo;t waste maintainers\u0026rsquo; time on your KPI grabbing patches (AKA, don\u0026rsquo;t be a KPI jerk)
`}),e.add({id:27,href:"/deancn/wiki/kube/1-kubernetes-on-macOS/",title:"1 Kubernetes on MAC Os",section:"Cloud Navtive",content:` Minikube Installation # Requirements # Minikube requires that VT-x/AMD-v virtualization is enabled in BIOS. To check that this is enabled on OSX / macOS run:
...
`}),e.add({id:28,href:"/deancn/wiki/kube/2-ingress-exmaple/",title:"2 Ingress Exmaple",section:"Cloud Navtive",content:` Kubernetes Ingress with Nginx Example # refer
...
`}),e.add({id:39,href:"/deancn/wiki/linux/shell/tools/",title:"Tools",section:"Shell",content:` Command for Developer # 收藏一些很好的开发工具。
brew (MacOS) # 查看可以升级的软件 brew outdated
CI (Continuous Integration) # github cli
terraform
docker # sudo docker exec -it -e COLUMNS=\$(tput cols) -e LINES=\$(tput lines) airdb/go bash tmux: 一个优秀的终端复用软件，类似GNU Screen jq: Command-line JSON processor, https://github.com/stedolan/jq `}),e.add({id:40,href:"/deancn/wiki/linux/shell/vim/",title:"Vim",section:"Shell",content:` VIM # vundle 搭建很方便
...
有问题的同学，可以随时联系
`}),e.add({id:41,href:"/deancn/wiki/osinit/mac-faq/",title:"MAC Faq",section:"OS Setup",content:` Mac 常见问题处理 # 修改主机名 # sudo scutil \u0026ndash;set HostName 新的主机名
...
`}),e.add({id:42,href:"/deancn/wiki/osinit/ubuntu-faq/",title:"Ubuntu Faq",section:"OS Setup",content:" Ubuntu FAQ # 更新证书链问题 # apt list --installed | grep ca-certificates apt-get update DEBIAN_FRONTEND=noninteractive apt-get install -y --reinstall ca-certificates "}),e.add({id:43,href:"/deancn/wiki/part1/4-ways-to-learn/",title:"4 Ways to Learn",section:"工作和生活效率",content:` 四种学习方法 (来源：搜狐, 侵删) # 一、费曼学习法： # 费曼学习法的灵感源于诺贝尔物理奖获得者理查德费曼（Richard Feynman），运用费曼技巧,你只需花上20分钟就能深入理解知识点,而且记忆深刻,难以遗忘。知识有两种类型，我们绝大多数人关注的都是错误的那类。第一类知识注重了解某个事物的名称。第二类知识注重了解某件事物。这可不是一回事儿。著名的诺贝尔物理学家理查德·费曼（Richard Feynman）能够理解这二者间的差别，这也是他成功最重要的原因之一。事实上，他创造了一种学习方法，确保他会比别人对事物了解得更透彻。
运用步骤：
...
`}),e.add({id:44,href:"/deancn/wiki/part1/efficient-communication/",title:"Efficient Communication",section:"工作和生活效率",content:` 高效沟通 # Efficient communication
...
挽留离职员工 # 实在挽留不成的时候，也要注意经营好这份情意。 劝退员工 # 任何人都应该有可以纠正错误的机会，公司应该给员工这样的机会，员工也应该给公司同样的机会。 跟客户沟通 # 吸引客户的兴趣、帮客户发现问题和管理客户的期望。 跟老板沟通 # 了解你的的老板，赢得老板的信任，管理老板的期望，非暴力\u0026quot;怼\u0026quot; 老板。 `}),e.add({id:45,href:"/deancn/wiki/part1/m1-ability/",title:"M1 Ability",section:"工作和生活效率",content:` 快速提升一线管理者的战斗力 # 1. 认知升级 # 如何学习优秀管理者的底层思维模型？ 何为优秀的管理者？ 何为优秀的 Leader? 如何做到阴阳同体，Leader 的二维修炼？ 领导力如何快速修炼？
...
目标：总结出可持续拿结果的方法
`}),e.add({id:46,href:"/deancn/wiki/part1/README/",title:"Readme",section:"工作和生活效率",content:` 公共部分 # 公共部分内容
`}),e.add({id:47,href:"/deancn/wiki/part2/cicd/",title:"Cicd",section:"Part2",content:` Code Wiki # Code Part # dev
`}),e.add({id:48,href:"/deancn/wiki/part2/gateway/",title:"Gateway",section:"Part2",content:` Gateway # 《深入理解BFE》
智能网关趋势 # HTTPS
Quic
...
`}),e.add({id:49,href:"/deancn/wiki/part2/kube/",title:"Kube",section:"Part2",content:` Code Wiki # Code Part # dev
`}),e.add({id:50,href:"/deancn/wiki/part2/linux/",title:"Linux",section:"Part2",content:` Linux Wiki # Basic # Linux
`}),e.add({id:51,href:"/deancn/wiki/part2/ops/",title:"Ops",section:"Part2",content:` 运维 # 接入 # 四层接入
七层接入
CDN
IP 库
Nginx
...
人脸识别
自然语言处理 NLP
`}),e.add({id:52,href:"/deancn/wiki/part2/osinit/",title:"Osinit",section:"Part2",content:` OS Init # 主机容器初始化 # [osinit]https://init.airdb.host/
osinit Repo
软件包管理 # Ubunut PPA ( https://launchpad.net/~airdb)
sudo apt-add-repository ppa:airdb/adb
`}),e.add({id:53,href:"/deancn/wiki/part2/README/",title:"Readme",section:"Part2",content:` Application 性能优化 # Linux 底层优化 # CPU￼ # 进程和 CPU 原理
...
`}),e.add({id:56,href:"/deancn/wiki/part4/defensive-programming/",title:"Defensive Programming",section:"Part4",content:` Defensive Programming # Reference # http://teaching.csse.uwa.edu.au/units/CITS1001/handouts/L14-defensive-programming.pdf
...
`}),e.add({id:57,href:"/deancn/wiki/part4/exec_base64/",title:"Exec Base64",section:"Part4",content:`Base64 入侵
...
`}),e.add({id:58,href:"/deancn/wiki/saas/",title:"Saas",section:"Wikis",content:` Reference # erad
erda doc
Wakatime
