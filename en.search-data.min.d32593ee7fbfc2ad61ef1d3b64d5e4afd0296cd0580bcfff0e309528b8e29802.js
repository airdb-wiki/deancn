"use strict";(function(){const t={cache:!0};t.doc={id:"id",field:["title","content"],store:["title","href","section"]};const e=FlexSearch.create("balance",t);window.bookSearchIndex=e,e.add({id:0,href:"/deancn/wiki/part1/",title:"工作和生活效率",section:"Wikis",content:` HPBP # HRBP 方法论 # HRBP 方法论：阿里Review怎么玩？
`}),e.add({id:1,href:"/deancn/wiki/kube/",title:"Cloud Navtive",section:"Wikis",content:` Overview # Hack with kube.
Kubernetes 源码阅读 - 田飞雨
`}),e.add({id:2,href:"/deancn/wiki/part3/",title:"Serverless 实战 - 无服务器编程",section:"Wikis",content:` Serverless 实战 - 无服务器编程 # Serverless 是什么？ # Serverless 架构是不是就不要服务器了？回答这个问题，我们需要了解下 Serverless 是什么。
...
`}),e.add({id:5,href:"/deancn/wiki/kube/2-ingress-exmaple/",title:"2 Ingress Exmaple",section:"Cloud Navtive",content:` Kubernetes Ingress with Nginx Example # refer
...
`}),e.add({id:6,href:"/deancn/wiki/kube/5-helm/",title:"5 Helm",section:"Cloud Navtive",content:` Helm V3 Installation # Requirements # helm repo add stable https://kubernetes-charts.storage.googleapis.com
install Mysql # mysql
helm install mysql --set mysqlRootPassword=secret stable/mysql MYSQL_ROOT_PASSWORD=\$(kubectl get secret --namespace default mysql -o jsonpath=\u0026#34;{.data.mysql-root-password}\u0026#34; | base64 --decode; echo) kubectl port-forward svc/mysql 3306 `}),e.add({id:7,href:"/deancn/wiki/kube/9-faq/",title:"9 Faq",section:"Cloud Navtive",content:` Kube \u0026amp; Helm FAQ # Minikube # (How to completely purge minikube config or reset IP back to 192.168.99.100)[https://stackoverflow.com/questions/53871053/how-to-completely-purge-minikube-config-or-reset-ip-back-to-192-168-99-100] ~/Library/VirtualBox/HostInterfaceNetworking-vboxnet0-Dhcpd.leases
`}),e.add({id:8,href:"/deancn/wiki/kube/helm/",title:"Helm",section:"Cloud Navtive",content:`Airdb Helm
Helm Package Repo. Heml index.yaml
Usage # helm repo add airdb https://www.airdb.com/helm/ helm repo update helm search repo helm/mychart helm install airdb/mychat Helm V3 Env and Search # helm env helm search repo airdb Create or Update Helm Chart Repo # git clone https://github.com/airdb/helm/ cd helm helm repo index ./ --url https://www.airdb.com/helm/ helm package mychart Update the index.yaml file based on your chart. helm repo index --merge index.yaml --url https://www.airdb.com/helm/ . Other Repos # helm repo add stable https://kubernetes-charts.storage.googleapis.com `}),e.add({id:9,href:"/deancn/wiki/part1/4-ways-to-learn/",title:"4 Ways to Learn",section:"工作和生活效率",content:` 四种学习方法 (来源：搜狐, 侵删) # 一、费曼学习法： # 费曼学习法的灵感源于诺贝尔物理奖获得者理查德费曼（Richard Feynman），运用费曼技巧,你只需花上20分钟就能深入理解知识点,而且记忆深刻,难以遗忘。知识有两种类型，我们绝大多数人关注的都是错误的那类。第一类知识注重了解某个事物的名称。第二类知识注重了解某件事物。这可不是一回事儿。著名的诺贝尔物理学家理查德·费曼（Richard Feynman）能够理解这二者间的差别，这也是他成功最重要的原因之一。事实上，他创造了一种学习方法，确保他会比别人对事物了解得更透彻。
运用步骤：
...
`}),e.add({id:10,href:"/deancn/wiki/part1/efficient-communication/",title:"Efficient Communication",section:"工作和生活效率",content:` 高效沟通 # Efficient communication
...
挽留离职员工 # 实在挽留不成的时候，也要注意经营好这份情意。 劝退员工 # 任何人都应该有可以纠正错误的机会，公司应该给员工这样的机会，员工也应该给公司同样的机会。 跟客户沟通 # 吸引客户的兴趣、帮客户发现问题和管理客户的期望。 跟老板沟通 # 了解你的的老板，赢得老板的信任，管理老板的期望，非暴力\u0026quot;怼\u0026quot; 老板。 `}),e.add({id:11,href:"/deancn/wiki/part1/m1-ability/",title:"M1 Ability",section:"工作和生活效率",content:` 快速提升一线管理者的战斗力 # 1. 认知升级 # 如何学习优秀管理者的底层思维模型？ 何为优秀的管理者？ 何为优秀的 Leader? 如何做到阴阳同体，Leader 的二维修炼？ 领导力如何快速修炼？
...
目标：总结出可持续拿结果的方法
`}),e.add({id:12,href:"/deancn/wiki/part1/README/",title:"Readme",section:"工作和生活效率",content:` 公共部分 # 公共部分内容
`}),e.add({id:13,href:"/deancn/wiki/part2/cicd/",title:"Cicd",section:"Part2",content:` Code Wiki # Code Part # dev
`}),e.add({id:14,href:"/deancn/wiki/part2/gateway/",title:"Gateway",section:"Part2",content:` Gateway # 《深入理解BFE》
智能网关趋势 # HTTPS
Quic
...
`}),e.add({id:15,href:"/deancn/wiki/part2/kube/",title:"Kube",section:"Part2",content:` Code Wiki # Code Part # dev
`}),e.add({id:16,href:"/deancn/wiki/part2/linux/",title:"Linux",section:"Part2",content:` Linux Wiki # Basic # Linux
`}),e.add({id:17,href:"/deancn/wiki/part2/ops/",title:"Ops",section:"Part2",content:` 运维 # 接入 # 四层接入
七层接入
CDN
IP 库
Nginx
...
人脸识别
自然语言处理 NLP
`}),e.add({id:18,href:"/deancn/wiki/part2/osinit/",title:"Osinit",section:"Part2",content:` OS Init # 主机容器初始化 # [osinit]https://init.airdb.host/
osinit Repo
软件包管理 # Ubunut PPA ( https://launchpad.net/~airdb)
sudo apt-add-repository ppa:airdb/adb
`}),e.add({id:19,href:"/deancn/wiki/part2/README/",title:"Readme",section:"Part2",content:` Application 性能优化 # Linux 底层优化 # CPU￼ # 进程和 CPU 原理
...
`}),e.add({id:22,href:"/deancn/wiki/part4/defensive-programming/",title:"Defensive Programming",section:"Part4",content:` Defensive Programming # Reference # http://teaching.csse.uwa.edu.au/units/CITS1001/handouts/L14-defensive-programming.pdf
...
`}),e.add({id:23,href:"/deancn/wiki/part4/exec_base64/",title:"Exec Base64",section:"Part4",content:`Base64 入侵
...
`}),e.add({id:24,href:"/deancn/wiki/saas/",title:"Saas",section:"Wikis",content:` Reference # erad
erda doc
Wakatime
`}),e.add({id:25,href:"/deancn/wiki/part1/okr/",title:"OKR",section:"工作和生活效率",content:`通过对 OKR 的学习和理解，应用到工作和生活，以此提升自我能力，提升家庭幸福感。
...
`}),e.add({id:26,href:"/deancn/wiki/pm/",title:"产品学习",section:"Wikis",content:" 产品学习 # 订单系统：从0到1设计思路 "}),e.add({id:27,href:"/deancn/wiki/emotion/l1/",title:"内卷问题",section:"情感训练",content:` 分类 # 工作内卷 婚姻内卷 人物分析 # p1 - 王开心
...
总结：
1. 接纳自己，适当认怂 2. 必要和别人比，复盘 3. 身心愉悦 4. 以终为始 5. 放掉输赢思维 6. 关注自己能改变的， 向内提升，影响别人，不要试图改变 7. 内卷如弹簧 8. 提高自己的人生维度， 降维打击 9. 材质不同，发挥优势，弥补短板 10. 长期主义者，个人建议多做做公益 11. 外面没有别人，只有自己，先成人，再成事 `}),e.add({id:28,href:"/deancn/wiki/part4/ids/indentify-proxy/",title:"如何识别代理",section:"用户行为安全",content:` 代理ip常见的一些功能 # 1.突破自身ip访问限制，现在有许多的网站都对ip地址访问进行了限制，这时则可以通过代理ip来突破限制，让自己进入网站。
