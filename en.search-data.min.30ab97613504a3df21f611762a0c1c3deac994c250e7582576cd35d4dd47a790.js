"use strict";(function(){const t={cache:!0};t.doc={id:"id",field:["title","content"],store:["title","href","section"]};const e=FlexSearch.create("balance",t);window.bookSearchIndex=e,e.add({id:0,href:"/deancn/wiki/dev/",title:"Development Handbook",section:"Wikis",content:` 开发手册 # 这是开发手册
...
`}),e.add({id:1,href:"/deancn/wiki/dev/go/",title:"Golang",section:"Development Handbook",content:` 学习资料 # Golang Gin 实践 gin源码分析 Golang 常见问题收录 # Golang 常见问题 # Go sync.Pool 浅析 [典藏版]Golang调度器GPM原理与调度全分析 开源项目 # Plugin 热更新问题 解析范式 # Go Channel的实现
G-P-M模型
G: 表示goroutine，存储了goroutine的执行stack信息、goroutine状态以及goroutine的任务函数等；另外G对象是可以重用的。 P: 表示逻辑processor，P的数量决定了系统内最大可并行的G的数量（前提：系统的物理cpu核数\u0026gt;=P的数量）；P的最大作用还是其拥有的各种G对象队列、链表、一些cache和状态。 M: M代表着真正的执行计算资源。在绑定有效的p后，进入schedule循环；而schedule循环的机制大致是从各种队列、p的本地队列中获取G，切换到G的执行栈上并执行G的函数，调用goexit做清理工作并回到m，如此反复。M并不保留G状态，这是G可以跨M调度的基础。 `}),e.add({id:2,href:"/deancn/wiki/part1/",title:"工作和生活效率",section:"Wikis",content:` HPBP # HRBP 方法论 # HRBP 方法论：阿里Review怎么玩？
`}),e.add({id:3,href:"/deancn/wiki/dev/coding-style/",title:"Coding Style",section:"Development Handbook",content:` Coding Style # Code Review # 如何在团队中做好Code Review
13 Practices for Better Code Reviews
`}),e.add({id:4,href:"/deancn/wiki/linux/",title:"Linux",section:"Wikis",content:` Linux Practice # 适用人群 # 建议人群：有一定 Linux 使用经验。
序语 # 本该是我在日常工作中使用到的一些 linux 小技巧或实践，不一定对每一个人有用，但应该会有一些参考意义。
如果你也有简单易用的技巧，欢迎 PR，一起成长吧！
`}),e.add({id:5,href:"/deancn/wiki/osinit/mac/",title:"MAC",section:"OS Setup",content:" Mac 初始化 # Homebrew 安装 # https://docs.brew.sh/Installation\n/bin/bash -c \u0026#34;$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/master/install.sh)\u0026#34; 安装常用软件 # brew install wget brew install nodejs brew install tree brew install jq brew install hugo brew install gh brew install golangci-lint # readlink brew install coreutils "}),e.add({id:6,href:"/deancn/wiki/osinit/ubuntu/",title:"Ubuntu",section:"OS Setup",content:" Ubuntu 初始化 # "}),e.add({id:7,href:"/deancn/wiki/kube/",title:"Cloud Navtive",section:"Wikis",content:` Overview # Hack with kube.
Kubernetes 源码阅读 - 田飞雨
`}),e.add({id:8,href:"/deancn/wiki/dev/github/",title:"Git and Github",section:"Development Handbook",content:` Git and Github 使用 # Git Command: # 将本地分支推送到远程主干分支
git push origin dev:main Maintain a repo without permission git remote add upstream https://github.com/bfenetworks/bfe.git git fetch upstream git checkout develop git merge upstream/develop git rebase upstream/develop Refer: https://docs.github.com/en/github/collaborating-with-issues-and-pull-requests/syncing-a-fork git config setting git config --global core.hooksPath .github/hooks git config --global core.excludefile .gitignore_global git config --global url.\u0026#39;ssh://git@github.com\u0026#39;.insteadOf https://github.com Github Commit. For Close Github Issue, commit message should as follow: close #x closes #x closed #x fix #x fixes #x fixed #x resolve #x resolves #x resolved #x add new quick sort algorithm, fixes #4, resolve #6, closed #12 git submodule update --init -f vendor/github.com/mholt/caddy git remote add ups https://github.com/airdb/b git push -u ups local_branch:master -f Github Command Line brew install github/gh/gh gh --repo bbhj/lbs issue status gh --repo bbhj/lbs issue view 1 Delete branch or tag git branch -D dev git push origin --delete dev git tag -d v1.0 git push --delete origin v1.0 `}),e.add({id:9,href:"/deancn/wiki/osinit/",title:"OS Setup",section:"Wikis",content:" 系统初始化 # 安装常用工具软件及开发环境 初始化配置 安装 Agent "}),e.add({id:10,href:"/deancn/wiki/part3/",title:"Serverless 实战 - 无服务器编程",section:"Wikis",content:` Serverless 实战 - 无服务器编程 # Serverless 是什么？ # Serverless 架构是不是就不要服务器了？回答这个问题，我们需要了解下 Serverless 是什么。
...
实战 5: 六边形架构
`}),e.add({id:11,href:"/deancn/wiki/dev/coding-style/document/",title:"Document",section:"Coding Style",content:` 中文文案排版指北 # 统一中文文案、排版的相关用法，降低团队成员之间的沟通成本，增强网站气质。
...
Workflow:
action.yml 中配置, 启用自定义的 action , 增加 input 变量作为 docker run 参数 调用自定义的 action , 从 env 中获取 GITHUB_EVENT_PATH, 读取 event.json 全部内容 代码中进行自定义操作 设置输出, 如 echo \u0026ldquo;::set-output name=time::\$time\u0026rdquo; `}),e.add({id:15,href:"/deancn/wiki/dev/github/fork/",title:"Fork",section:"Git and Github",content:` Github 技巧 # Gitlab 或 Github Fork 后如何同步源的新更新内容？ # 给 fork 配置远程库 # 查看远程状态 git remote -v
...
`}),e.add({id:16,href:"/deancn/wiki/dev/github/git_branch_manage/",title:"Git Branch Manage",section:"Git and Github",content:` Git 分支的约束和管理 # 来源： dxyoo7
...
`}),e.add({id:17,href:"/deancn/wiki/dev/github/how_to_github_pull_request/",title:"How to Github Pull Request",section:"Git and Github",content:` 如何参与贡献: # Github 提交 PR, 地址: github.com/airdb/fun Github 提交 issues, 地址: github.com/airdb/fun/issues 写邮件给 info@airdb.com Github 如何提交 PR # step 1:
...
`}),e.add({id:18,href:"/deancn/wiki/dev/github/issue/",title:"Issue",section:"Git and Github",content:` Github Issue 的用法 # Reference
...
`}),e.add({id:21,href:"/deancn/wiki/dev/go/pprof/memory_leak/",title:"Memory Leak",section:"Golang",content:` go pprof与线上事故：一次成功的定位与失败的复现
...
`}),e.add({id:22,href:"/deancn/wiki/dev/go/serverless-tencent/",title:"Serverless Tencent",section:"Golang",content:` 腾讯云 Serverless # 讨论区： https://github.com/serverless/serverless-tencent/discussions
...
部署 # SERVERLESS_PLATFORM_VENDOR=tencent GLOBAL_ACCELERATOR_NA=true sls deploy --stage test 查看日志 # SERVERLESS_PLATFORM_VENDOR=tencent GLOBAL_ACCELERATOR_NA=true sls logs --tail --stage test `}),e.add({id:23,href:"/deancn/wiki/dev/go/tools/",title:"Tools",section:"Golang",content:` Golang Command Line tools # Linux pipes in Golang
使用Go语言在MacOS创建一个自定义的命令行工具
你应该定期更新 Homebrew
`}),e.add({id:24,href:"/deancn/wiki/dev/standard/case/",title:"Case",section:"Development Handbook",content:` 好好写代码 # Bad Cases # Please don\u0026rsquo;t waste maintainers\u0026rsquo; time on your KPI grabbing patches (AKA, don\u0026rsquo;t be a KPI jerk)
`}),e.add({id:25,href:"/deancn/wiki/kube/1-kubernetes-on-macOS/",title:"1 Kubernetes on MAC Os",section:"Cloud Navtive",content:` Minikube Installation # Requirements # Minikube requires that VT-x/AMD-v virtualization is enabled in BIOS. To check that this is enabled on OSX / macOS run:
...
`}),e.add({id:26,href:"/deancn/wiki/kube/2-ingress-exmaple/",title:"2 Ingress Exmaple",section:"Cloud Navtive",content:` Kubernetes Ingress with Nginx Example # refer
...
`}),e.add({id:37,href:"/deancn/wiki/linux/shell/tools/",title:"Tools",section:"Shell",content:` Command for Developer # 收藏一些很好的开发工具。
brew (MacOS) # 查看可以升级的软件 brew outdated
CI (Continuous Integration) # github cli
terraform
docker # sudo docker exec -it -e COLUMNS=\$(tput cols) -e LINES=\$(tput lines) airdb/go bash tmux: 一个优秀的终端复用软件，类似GNU Screen jq: Command-line JSON processor, https://github.com/stedolan/jq `}),e.add({id:38,href:"/deancn/wiki/linux/shell/vim/",title:"Vim",section:"Shell",content:` VIM # vundle 搭建很方便
...
有问题的同学，可以随时联系
`}),e.add({id:39,href:"/deancn/wiki/osinit/mac-faq/",title:"MAC Faq",section:"OS Setup",content:` Mac 常见问题处理 # 修改主机名 # sudo scutil \u0026ndash;set HostName 新的主机名
...
`}),e.add({id:40,href:"/deancn/wiki/osinit/ubuntu-faq/",title:"Ubuntu Faq",section:"OS Setup",content:" Ubuntu FAQ # 更新证书链问题 # apt list --installed | grep ca-certificates apt-get update DEBIAN_FRONTEND=noninteractive apt-get install -y --reinstall ca-certificates "}),e.add({id:41,href:"/deancn/wiki/part1/4-ways-to-learn/",title:"4 Ways to Learn",section:"工作和生活效率",content:` 四种学习方法 (来源：搜狐, 侵删) # 一、费曼学习法： # 费曼学习法的灵感源于诺贝尔物理奖获得者理查德费曼（Richard Feynman），运用费曼技巧,你只需花上20分钟就能深入理解知识点,而且记忆深刻,难以遗忘。知识有两种类型，我们绝大多数人关注的都是错误的那类。第一类知识注重了解某个事物的名称。第二类知识注重了解某件事物。这可不是一回事儿。著名的诺贝尔物理学家理查德·费曼（Richard Feynman）能够理解这二者间的差别，这也是他成功最重要的原因之一。事实上，他创造了一种学习方法，确保他会比别人对事物了解得更透彻。
运用步骤：
...
`}),e.add({id:42,href:"/deancn/wiki/part1/efficient-communication/",title:"Efficient Communication",section:"工作和生活效率",content:` 高效沟通 # Efficient communication
...
挽留离职员工 # 实在挽留不成的时候，也要注意经营好这份情意。 劝退员工 # 任何人都应该有可以纠正错误的机会，公司应该给员工这样的机会，员工也应该给公司同样的机会。 跟客户沟通 # 吸引客户的兴趣、帮客户发现问题和管理客户的期望。 跟老板沟通 # 了解你的的老板，赢得老板的信任，管理老板的期望，非暴力\u0026quot;怼\u0026quot; 老板。 `}),e.add({id:43,href:"/deancn/wiki/part1/m1-ability/",title:"M1 Ability",section:"工作和生活效率",content:` 快速提升一线管理者的战斗力 # 1. 认知升级 # 如何学习优秀管理者的底层思维模型？ 何为优秀的管理者？ 何为优秀的 Leader? 如何做到阴阳同体，Leader 的二维修炼？ 领导力如何快速修炼？
...
目标：总结出可持续拿结果的方法
`}),e.add({id:44,href:"/deancn/wiki/part1/README/",title:"Readme",section:"工作和生活效率",content:` 公共部分 # 公共部分内容
`}),e.add({id:45,href:"/deancn/wiki/part2/cicd/",title:"Cicd",section:"Part2",content:` Code Wiki # Code Part # dev
`}),e.add({id:46,href:"/deancn/wiki/part2/gateway/",title:"Gateway",section:"Part2",content:` Gateway # 《深入理解BFE》
智能网关趋势 # HTTPS
Quic
...
`}),e.add({id:47,href:"/deancn/wiki/part2/kube/",title:"Kube",section:"Part2",content:` Code Wiki # Code Part # dev
`}),e.add({id:48,href:"/deancn/wiki/part2/linux/",title:"Linux",section:"Part2",content:` Linux Wiki # Basic # Linux
`}),e.add({id:49,href:"/deancn/wiki/part2/ops/",title:"Ops",section:"Part2",content:` 运维 # 接入 # 四层接入
七层接入
CDN
IP 库
Nginx
...
人脸识别
自然语言处理 NLP
`}),e.add({id:50,href:"/deancn/wiki/part2/osinit/",title:"Osinit",section:"Part2",content:` OS Init # 主机容器初始化 # [osinit]https://init.airdb.host/
osinit Repo
软件包管理 # Ubunut PPA ( https://launchpad.net/~airdb)
sudo apt-add-repository ppa:airdb/adb
`}),e.add({id:51,href:"/deancn/wiki/part2/README/",title:"Readme",section:"Part2",content:` Application 性能优化 # Linux 底层优化 # CPU￼ # 进程和 CPU 原理
...
`}),e.add({id:54,href:"/deancn/wiki/part4/defensive-programming/",title:"Defensive Programming",section:"Part4",content:` Defensive Programming # Reference # http://teaching.csse.uwa.edu.au/units/CITS1001/handouts/L14-defensive-programming.pdf
...
`}),e.add({id:55,href:"/deancn/wiki/part4/exec_base64/",title:"Exec Base64",section:"Part4",content:`Base64 入侵
...
`}),e.add({id:56,href:"/deancn/wiki/saas/",title:"Saas",section:"Wikis",content:` Reference # erad
erda doc
Wakatime
`}),e.add({id:57,href:"/deancn/wiki/dev/vuejs/vue-chrome-extension/",title:"Chrome 插件制作",section:"Development Handbook",content:` # 创建项目 # vue create --preset kocal/vue-web-extension my-extension cd my-extension npm run serve 载入 # chrome 浏览器输入 chrome://extensions, 加载插件选择 my-extension/dist 目录。 加载成功即可生效。
参考 # https://developer.chrome.com/docs/extensions/reference/commands/
`}),e.add({id:58,href:"/deancn/wiki/dev/standard/sec/",title:"DevSecOps",section:"Development Handbook",content:` 面向开发人员梳理的代码安全指南 # Tencent: https://github.com/tencent/secguide
原文: https://mp.weixin.qq.com/s/yzg5uVnoJDTyaH2Wn8Vo7w
设计理念 # 代码安全指南的内容呈树状结构展开，共分5层，如下：
`}),e.add({id:59,href:"/deancn/wiki/part1/okr/",title:"OKR",section:"工作和生活效率",content:`通过对 OKR 的学习和理解，应用到工作和生活，以此提升自我能力，提升家庭幸福感。
...
`}),e.add({id:60,href:"/deancn/wiki/iot/raspberry/",title:"Raspberry 树莓派",section:"Wikis",content:` 树莓派系统烧录 # https://stepneverstop.github.io/burn-system2raspberry-in-macos.html
系统配置 # 重新插拔usb
...
总结：
1. 接纳自己，适当认怂 2. 必要和别人比，复盘 3. 身心愉悦 4. 以终为始 5. 放掉输赢思维 6. 关注自己能改变的， 向内提升，影响别人，不要试图改变 7. 内卷如弹簧 8. 提高自己的人生维度， 降维打击 9. 材质不同，发挥优势，弥补短板 10. 长期主义者，个人建议多做做公益 11. 外面没有别人，只有自己，先成人，再成事 `}),e.add({id:63,href:"/deancn/wiki/part4/ids/indentify-proxy/",title:"如何识别代理",section:"用户行为安全",content:` 代理ip常见的一些功能 # 1.突破自身ip访问限制，现在有许多的网站都对ip地址访问进行了限制，这时则可以通过代理ip来突破限制，让自己进入网站。
...
24、熟悉MySQL锁优化吗？
25、熟悉哪些MySQL调优策略？
`}),e.add({id:68,href:"/deancn/wiki/part1/time-management/",title:"时间管理",section:"工作和生活效率",content:` 时间管理 # 如何一天完成4个人的工作量?
07:00 - 09:00 是大脑的黄金时间, 可以用来写文章，看书，学外语。 09:00 - 12:00 适合罐头工作法, 找一个安静无打扰的空间, 保持高度的专注。 13:00 - 16:00 是人在一天当中最困的时候，可以适当小睡, 也可以通过变化工作场所来恢复。 16:00 - 18:00 人会升起一种紧迫感, 想着一天不能就这样结束了，进入高效工作。 18:00 - 19:00 适合通过运动来让大脑重启。 19:00 - 21:00 是大脑的第二个黄金时间, 适合再做一些自我投资。 21:00 以后为了不失眠，还是尽早放松吧。 `}),e.add({id:69,href:"/deancn/wiki/dev/mq/",title:"消息队列",section:"Development Handbook",content:` 高性能万亿级消息吞吐Pulsar设计原理
`}),e.add({id:70,href:"/deancn/wiki/part4/crawler/",title:"爬虫",section:"Part4",content:` https://docs.crawlab.cn/zh/Spider/ConfigurableSpider.html
https://github.com/gocolly/colly https://github.com/crawlab-team/crawlab
`}),e.add({id:71,href:"/deancn/wiki/training/1-setup-os-and-environment/",title:"环境设置",section:"训练板块",content:" Mac # brew Windows # ubuntu shell and network Common # github.com vscode golang hugo (for recording documents) "}),e.add({id:72,href:"/deancn/wiki/part4/ids/",title:"用户行为安全",section:"Part4",content:` 爬虫检测技术 # 行为检测: 通过分析网页上用户的操作（鼠标的移动、点击、滚动行为和浏览行为）来判断操作者是否是机器控制的网络爬虫。
...
爬虫风险管理
基于Web的爬虫识别算法
`}),e.add({id:73,href:"/deancn/wiki/linux/network/",title:"网络",section:"Linux",content:`姜知笔记: https://www.gingerdoc.com/
`}),e.add({id:74,href:"/deancn/wiki/training/",title:"训练板块",section:"Wikis",content:`目标：练习常用的技能
`})})()